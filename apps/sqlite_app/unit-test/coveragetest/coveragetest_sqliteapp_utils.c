...
void Test_SQLITE_APP_TblValidationFunc(void)
{
    /*
     * Test Case For:
    ...
    UtAssert_INT32_EQ(SQLITE_APP_TblValidationFunc(&TestTblData), CFE_SUCCESS);

    /* error case should return SQLITE_APP_TABLE_OUT_OF_RANGE_ERR_CODE */
    TestTblData.Int1 = 1 + SQLITE_APP_TBL_ELEMENT_1_MAX;
    UtAssert_INT32_EQ(SQLITE_APP_TblValidationFunc(&TestTblData), SQLITE_APP_TABLE_OUT_OF_RANGE_ERR_CODE);
}

void Test_SQLITE_APP_GetCrc(void)
{
    /*
     * Test Case For:
...
{
    ADD_TEST(SQLITE_APP_TblValidationFunc);
    ADD_TEST(SQLITE_APP_GetCrc);
}
